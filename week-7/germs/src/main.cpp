...
void testcase(int n)
{
  ...
  for (auto vertex: triangulation.finite_vertex_handles()) {
    auto neighbor = CGAL::nearest_neighbor(triangulation, vertex);
    
    long n_dist = std::numeric_limits<long>::max(); 
    // Check if the neighbour exists
    if (neighbor != nullptr) {
        n_dist = CGAL::squared_distance(vertex->point(), neighbor->point());
    }
    
    long x = vertex->point().x(), y = vertex->point().y();
